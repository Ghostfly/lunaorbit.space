    ...
    <section class="text-gray-600 body-font relative">
      ...
      <div class="container px-5 py-24 mx-auto flex">
        <div class="lg:w-1/3 md:w-1/2 bg-white rounded-lg p-8 flex flex-col md:ml-auto w-full mt-10 md:mt-0 relative z-10 shadow-md">
          <h2 class="text-gray-900 text-lg mb-6 font-medium title-font">${msg('Contact us')}</h2>
          <form
            id="contact-form"
          ...
          >
            <div class="relative mb-4">
              <label for="email" class="leading-7 text-sm text-gray-600">${msg('Email')}</label>
            ...
            <div class="relative mb-4">
              <label for="message" class="leading-7 text-sm text-gray-600">${msg('Comment')}</label>
            ...
            <button type="submit" class="text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-blue-600 rounded text-lg">${msg('Send')}</button>
            <p id="status" class="font-semibold"></p>
            <p class="text-md mt-3">${msg('Find us on')} <a href="http://t.me/${config.telegram}" target="_blank" title="Telegram" class="text-gray-500 hover:text-indigo-500">Telegram</a> ${msg('or')} <a class="text-gray-500 hover:text-indigo-500" href="https://twitter.com/${config.twitter}" target="_blank" title="Twitter">Twitter.</p>
        </form>
        </div>
      </div>
    </section>
