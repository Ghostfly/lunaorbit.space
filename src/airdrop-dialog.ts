...
export class AirdropDialog extends Localized(LitElement) {
  static ANCAirdropCheckingURL = 'https://mantle.anchorprotocol.com/?__isClaimed';
  ...
  protected _mirAirdropCheckParams(): TerraQuery {
    return {
      ...
      "query": "query airdrop($address: String!, $network: String = \"TERRA\") {\n  airdrop(address: $address, network: $network)\n}\n"
    };
  }
  ...
  protected _ancAirdropCheckParams(): TerraQuery {
    return {
      ...
      query: "query __isClaimed($airdropContract: String!, $isClaimedQuery: String!) { isClaimed: WasmContractsContractAddressStore(ContractAddress: $airdropContract, QueryMsg: $isClaimedQuery) {    Result    __typename  }}"
    };
  }
              ...
              <div class="sm:flex sm:items-start">
                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                  ...
                  <div class="flex lg:w-2/3 w-full sm:flex-row flex-col mx-auto px-8 sm:space-x-4 sm:space-y-0 space-y-4 sm:px-0 items-end m-10">
                    ...
                    <button .disabled=${this.loading} class="${this.loading ? 'opacity-50 cursor-wait' : ''} text-white terra-bg border-0 py-2 px-8 rounded text-lg" @click=${() => this._checkAirdrops()}">${msg('Check')}</button>
                  </div>
                </div>
              </div>
